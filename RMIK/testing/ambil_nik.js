...
void (async function ()
{
    ...
    // 5. Format NIK → satu baris, pisah koma
    const nik_line = result.map(r => r.nik).join(",");

    // 6. Download TXT
    const download_txt = (content, filename) =>
    {
    ...
    const file_name = `data_odng_${START_YMD}_${END_YMD}.txt`;
    download_txt(nik_line, file_name);
    console.log(`💾 Download: ${file_name}`);

    // 7. Simpan ke window buat akses manual
    window.__odng_result__ = result;
    window.__odng_raw__ = unique_raw;
    window.__odng_nik__ = nik_line;
    console.log("📦 Akses manual: window.__odng_result__ | window.__odng_raw__ | window.__odng_nik__");
